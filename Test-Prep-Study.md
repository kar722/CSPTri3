...
4. Create a directory `_layouts` in the repository
   1. Rename `index.html` to `template.html` and move it into the `_layouts` directory
5. Open `template.html` and replace the `<p>CONTENT</p>` placeholder with {% raw %}`{{ content }}`{% endraw %} (this is [Jekyll](https://jekyllrb.com) syntax to grab the content from the MarkDown pages you will create)
6. Identify the navigation/button section of HTML
7. Copy one navigation/button item (probably a `<a href="">` or similar tag)
8. Insert this code at the top of the navigation/button item section:
